...
function getCapitolSVG() {
  ...
  return (
    document.querySelector("#capitol-map-svg") ||
...
function getAllowedTransitionStairKeys(graph, pathId) {
  ...
  if (!rule) return null;
  if (
    Array.isArray(rule.transitionStairKeys) &&
    rule.transitionStairKeys.length
  ) {
...
function shouldForceStairTransition(graph, startPathId, endPathId) {
  ...
  return Boolean(
    (startRule && startRule.enforceTransitions) ||
      (endRule && endRule.enforceTransitions)
  );
}
...
function findStairNodeByKeyAndVariant(graph, stairKey, variant) {
  return (
    findStairNodesBy(graph, (room, node) => {
...
function determineCandidateStairKeys(
  startGraph,
  startPathId,
  endGraph,
  endPathId
...
function parseFloorFromRoomId(roomId) {
  ...
  // Try standard room ID format first (room-12-1, stair_west_1-1, etc.)
  ...
  // For entrances (entrance_west_1, entrance_main_2, etc.), extract floor from suffix
  // entrance_west_1 → floor 1, entrance_main_2 → floor 2
  const entranceMatch = roomId.match(/(\d+)$/);
  if (entranceMatch) {
    const floorFromSuffix = parseInt(entranceMatch[1], 10);
    if (!Number.isNaN(floorFromSuffix)) {
      return floorFromSuffix;
    }
  }
  
  // Fallback: Check if room exists in any loaded floor graph
  for (const [floor, graph] of Object.entries(floorGraphCache)) {
    if (graph && graph.rooms && graph.rooms[roomId]) {
...
function decorateFloorGraph(data, floor) {
  ...
  if (graph.rooms && typeof graph.rooms === "object") {
    Object.values(graph.rooms).forEach((room) => {
      ...
      const legacyDoorPoints = Array.isArray(room.doorPoints)
        ? room.doorPoints
        : [];
  ...
  // Convert entrances to virtual rooms so they work identically to rooms with doorPoints
  // This allows entrances to participate in pathfinding using the same logic as regular rooms
  if (Array.isArray(graph.entrances) && graph.entrances.length > 0) {
    ...
    graph.entrances.forEach((entrance) => {
      ...
      // Create a virtual room object that mimics a room with doorPoints
      const virtualRoom = {
        type: 'entrance',
        roomId: entrance.id,  // CRITICAL: Add roomId for compatibility
        floor: entrance.floor || floor,  // CRITICAL: Store floor number for parseFloorFromRoomId fallback
        ...
        // Store original entrance data for reference
        entranceData: entrance
      };
      
      // Add to rooms object using entrance ID as room ID
      graph.rooms[entrance.id] = virtualRoom;
      
      console.log(`[decorateFloorGraph] Created virtual room for entrance: ${entrance.id}`, {
        floor: virtualRoom.floor,
        x: entrance.x,
        y: entrance.y,
        nearestPathId: entrance.nearestPathId
      });
...
function getStairNodesConnectingToFloor(graph, targetFloor) {
  if (!graph || !Array.isArray(graph.stairNodes)) return [];
  return graph.stairNodes.filter(
    (node) =>
      Array.isArray(node.room.connectsTo) &&
      node.room.connectsTo.includes(targetFloor)
  );
}
...
function getRequiredStairVariantForPath(graph, pathId) {
  ...
  return variantsByKey[firstKey]
    ? { stairKey: firstKey, variant: variantsByKey[firstKey] }
    : null;
}
...
function renderActiveRouteForFloor(floor) {
  ...
  // CRITICAL FIX: Clean SVG transforms before rendering to prevent corruption
  ...
  if (svg) {
    ...
    // Clean path group transforms BEFORE drawing new paths
    ...
    if (mainGroup) {
      ...
      if (pathGroup && pathGroup.hasAttribute("transform")) {
        console.log("🔧 renderActiveRouteForFloor: Removing corrupt transform from path group");
  ...
  const walkSegments = segments.filter(
    (segment) =>
      segment.type === "walk" && segment.points && segment.points.length
  );
  ...
  const selectionIds = [activeRoute.startRoomId, activeRoute.endRoomId].filter(
    Boolean
  );
...
function ensureRouteInstructionsPanel() {
  if (
    ...
    document.body.contains(routeInstructionsPanel)
  ) {
...
function renderRouteInstructions(route) {
  ...
  if (route.type === "multi-floor") {
    const stairNames =
      Array.isArray(route.stairSequence) && route.stairSequence.length
        ? [
            ...new Set(
              route.stairSequence.map((item) => describeStairKey(item.stairKey))
            ),
          ]
        : route.stairKey
        ? [describeStairKey(route.stairKey)]
        : [];
  ...
  (route.segments || []).forEach((segment) => {
    ...
    if (segment.type === "stair" && segment.floorSpan) {
      ...
      floorSpanLine.textContent = `Covers ${segment.floorSpan} floor${
        segment.floorSpan === 1 ? "" : "s"
      }.`;
  ...
  if (route.type === "multi-floor") {
    ...
    tip.textContent =
      "Switch floors using the floor selector when you arrive. The highlighted route will update automatically.";
...
function drawWalkablePath(walkablePath) {
  ...
  let pathData = "";
  console.log(
    `Generating path data for ${walkablePath.pathPoints.length} points`
  );
    ...
    points.forEach((point, index) => {
      ...
      if (point.isPano) {
        ...
        cameraIcon.setAttribute(
          "d",
          "M14 4h-1l-2-2h-2l-2 2h-1c-1.1 0-2 .9-2 2v8c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2v-8c0-1.1-.9-2-2-2zm-4 7c-1.65 0-3-1.35-3-3s1.35-3 3-3 3 1.35 3 3-1.35 3-3 3z"
        );
        cameraIcon.setAttribute("fill", "#ffffff");
        // Adjust transform to match explore.php marker sizing
        cameraIcon.setAttribute(
          "transform",
          `translate(${point.x - 8}, ${point.y - 8}) scale(0.8)`
        );
        ...
        marker.addEventListener("click", (event) => {
          ...
          document
            .querySelectorAll(".panorama-marker.active")
  ...
  for (const roomId in rooms) {
    ...
    if (entryPoints.length > 0 && room.style && room.style.pointMarker) {
      ...
      entryPoints.forEach((entryPoint, index) => {
        console.log(
          `Drawing entry point for ${roomId} at (${entryPoint.x}, ${entryPoint.y})`
        );

        // Create door icon group (styled like panorama markers)
        const marker = document.createElementNS(svgNS, "g");
        marker.setAttribute("class", "entry-point-marker");
        ...
        bgCircle.setAttribute("fill", "#F97316"); // Blue background
        bgCircle.setAttribute("stroke", "#ffffff");
        bgCircle.setAttribute("stroke-width", "1.5");
        ...
        doorIcon.setAttribute(
          "d",
          "M9 0L3 2V14H1V16H15V14H13V2H11V14H9V0ZM6.75 9C7.16421 9 7.5 8.55229 7.5 8C7.5 7.44772 7.16421 7 6.75 7C6.33579 7 6 7.44772 6 8C6 8.55229 6.33579 9 6.75 9Z"
        );
        doorIcon.setAttribute("fill", "#ffffff");
        // Adjust transform to center and scale the door icon
        doorIcon.setAttribute(
          "transform",
          `translate(${entryPoint.x - 8}, ${entryPoint.y - 8}) scale(1)`
        );
        doorIcon.setAttribute("class", "door-icon");
        doorIcon.style.pointerEvents = "none"; // Make icon non-interactive

        marker.appendChild(bgCircle);
        marker.appendChild(doorIcon);

        // Add hover effects - change to lighter blue
        marker.addEventListener("mouseenter", () => {
          bgCircle.setAttribute("fill", "#ffa05cff");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.MOBILE_MODE) {
    console.log(
      "[Pathfinding] MOBILE_MODE detected – skipping auto initPathfinding. Mobile layer drives path loading."
    );
  ...
  window.addEventListener("floorMapLoaded", (event) => {
    ...
    console.log(
      `Floor map loaded for floor ${floor}, initializing pathfinding...`
    );
...
function getEntryPointsForRoom(room) {
  ...
  // NEW: Check if this is the scanned start room with a specific door point
  if (typeof window !== 'undefined' && window.scannedStartDoorPoint) {
    const scannedData = window.scannedStartDoorPoint;
    const roomIdentifier = room.roomId || room.id || null;
    
    console.log('🔍 Checking if room matches scanned start:', {
      roomIdentifier,
      scannedLocation: scannedData.office?.location,
      scannedDoorIndex: scannedData.doorIndex,
      match: roomIdentifier === scannedData.office?.location
    });
    
    // If this room matches the scanned office's location, return only the scanned door point
    if (scannedData.office && scannedData.office.location === roomIdentifier) {
      console.log('🚪✅ MATCH! Using scanned door point as entry point:', scannedData.point, 'Door index:', scannedData.doorIndex);
      
      // Ensure the point is not marked as virtual so it passes the filter
      const doorPoint = {...scannedData.point, virtual: false};
      return [doorPoint];
    }
  }
  ...
  // Collect entry points from various possible properties
  if (Array.isArray(room.entryPoints) && room.entryPoints.length) {
    entryPoints = room.entryPoints;
  } else if (Array.isArray(room.doorPoints) && room.doorPoints.length) {
    entryPoints = room.doorPoints;
  } else if (room.doorPoint) {
    entryPoints = [room.doorPoint];
  }
  
  // If no entry points found, return empty array
  if (entryPoints.length === 0) return [];
  
  // Smart door filtering: Only apply in mobile context
  if (typeof window.isDoorActive === 'function' && room.roomId) {
    const roomId = room.roomId;
    
    // Special handling based on number of doors
    if (entryPoints.length === 1) {
      // SINGLE DOOR: Always allow pathfinding even if inactive
      // This ensures rooms with one door remain accessible
      const isActive = window.isDoorActive(roomId, 0);
      if (!isActive) {
        console.log(`⚠️  Single door ${roomId}-door-0 is inactive but allowing pathfinding`);
      }
      return entryPoints; // Return the single door regardless of status
    } else {
      // MULTIPLE DOORS: Filter out inactive doors, use only active ones
      const activeEntryPoints = entryPoints.filter((point, index) => {
        const isActive = window.isDoorActive(roomId, index);
        if (!isActive) {
          console.log(`🚫 Excluding inactive door: ${roomId}-door-${index} (${entryPoints.length} doors total)`);
        }
        return isActive;
      });
      
      // If all doors are inactive (edge case), return all as fallback
      if (activeEntryPoints.length === 0) {
        console.warn(`⚠️  All ${entryPoints.length} doors for ${roomId} are inactive! Using all as fallback.`);
        return entryPoints;
      }
      
      console.log(`✅ Using ${activeEntryPoints.length} active door(s) out of ${entryPoints.length} for ${roomId}`);
      return activeEntryPoints;
    }
  }
  
  // In admin context (floorPlan.php), return all entry points unfiltered
  return entryPoints;
}
...
function getMandatoryEntryPoint(graph, roomId) {
  ...
  if (rule.entryPointFloor && rule.entryPointFloor !== graph.floorNumber) {
    ...
    if (!entryFloorGraph) {
      console.warn(
        `Entry point floor ${rule.entryPointFloor} not loaded for room ${roomId}`
      );
      return null;
    }
    ...
    if (!entryRoom) {
      console.warn(
        `Mandatory entry point ${entryPointId} not found on floor ${rule.entryPointFloor} for room ${roomId}`
      );
      return null;
    }

    // Ensure entryRoom has roomId for door filtering
    if (!entryRoom.roomId) {
      entryRoom.roomId = entryPointId;
    }
  ...
  if (!entryRoom) {
    console.warn(
      `Mandatory entry point ${entryPointId} not found for room ${roomId}`
    );
    return null;
  }
...
function getRepresentativePointsForRoom(graph, roomId, limit = 6) {
  ...
  // Ensure room has roomId property for door filtering
  if (!room.roomId) {
    room.roomId = roomId;
  }
  ...
  entryPoints.slice(0, limit).forEach((pt) => {
    if (pt && typeof pt.x === "number" && typeof pt.y === "number") {
      const exists = points.some(
        (existing) => existing.x === pt.x && existing.y === pt.y
      );
      if (!exists) {
        points.push({ x: pt.x, y: pt.y });
      }
...
function augmentRouteWithRestrictedStart({
  route,
  startRoomId,
  startFloor,
  entryPoint,
  ...
  const startRoom =
    startGraph && startGraph.rooms ? startGraph.rooms[startRoomId] : null;
  ...
  const existingSegments = Array.isArray(route.segments)
  ...
  if (
    stairKey &&
    entryPoint.floor != null &&
    startFloor != null &&
    entryPoint.floor !== startFloor
  ) {
    const stairUsage = {
      stairKey,
    ...
    route.stairSequence = Array.isArray(route.stairSequence)
      ? route.stairSequence.slice()
      : [];
    const alreadyRecorded = route.stairSequence.some(
      (seq) =>
...
function augmentRouteWithRestrictedEnd({
  route,
  endRoomId,
  endGraph,
  entryPoint,
  ...
  const entryParsed = parseStairId(entryPoint.roomId);
  const stairKey =
    forcedStair?.stairKey ||
    entryPoint.room?.stairKey ||
    entryParsed?.key ||
    null;
...
function calculateSingleFloorRoute(graph, startRoomId, endRoomId) {
  ...
  // Ensure rooms have roomId property for door filtering
  if (!startRoom.roomId) {
    startRoom.roomId = startRoomId;
  }
  if (!endRoom.roomId) {
    endRoom.roomId = endRoomId;
  }
  
  console.log('🏠 Room IDs set:', {
    startRoomId: startRoom.roomId,
    endRoomId: endRoom.roomId,
    scannedStartDoorPointExists: !!(typeof window !== 'undefined' && window.scannedStartDoorPoint),
    scannedOfficeLocation: (typeof window !== 'undefined' && window.scannedStartDoorPoint) ? window.scannedStartDoorPoint.office?.location : 'N/A'
  });
  ...
  console.log(
    `[calculateSingleFloorRoute] Floor ${graph.floorNumber}: ${startRoomId} -> ${endRoomId}`,
    {
  ...
  if (
    ...
    if (entryPoint) {
      console.log(
        `Both rooms use same mandatory entry: ${entryPoint.roomId} on floor ${entryPoint.floor}`
      );
      ...
      if (entryPoint.floor !== graph.floorNumber) {
        console.log(
          `Entry point on different floor (${entryPoint.floor}), cannot route on same floor`
        );
  ...
  if (startRestriction && !endRestriction) {
    ...
    if (entryPoint) {
      console.log(
        `Start room ${startRoomId} restricted, routing from ${entryPoint.roomId} on floor ${entryPoint.floor} to ${endRoomId}`
      );

      // If entry point is on a different floor, return null to trigger multi-floor routing
      if (entryPoint.floor !== graph.floorNumber) {
        console.log(
          `Entry point on different floor (${entryPoint.floor}), triggering multi-floor route`
        );
        return null;
      }

      const routeFromEntry = calculateSingleFloorRoute(
        graph,
        entryPoint.roomId,
        endRoomId
      );
  ...
  if (!startRestriction && endRestriction) {
    ...
    if (entryPoint) {
      console.log(
        `End room ${endRoomId} restricted, routing from ${startRoomId} to ${entryPoint.roomId} on floor ${entryPoint.floor}`
      );
      ...
      const routeToEntry = calculateSingleFloorRoute(
        graph,
        startRoomId,
        entryPoint.roomId
      );
  ...
  console.log('🚪 Entry points retrieved:', {
    startRoom: startRoom.roomId,
    startDoors: startDoors,
    endRoom: endRoom.roomId,
    endDoors: endDoors
  });
  ...
  console.log('🚪 After virtual filter:', {
    realStartDoors: realStartDoors,
    realEndDoors: realEndDoors
  });

  if (!realStartDoors.length || !realEndDoors.length) {
    console.warn(
      `No real (non-virtual) entry points found for rooms on floor ${graph.floorNumber}`,
      {
        startRoomId,
        endRoomId,
  ...
  for (const startDoor of realStartDoors) {
    for (const endDoor of realEndDoors) {
      ...
      let startPath = startPathId
        ? graph.walkablePaths.find((p) => p.id === startPathId)
        : null;
      let endPath = endPathId
        ? graph.walkablePaths.find((p) => p.id === endPathId)
        : null;

      let startPathPoint = startPath
        ? findNearestPointOnPath(startDoor, startPath.pathPoints)
        : null;
      let endPathPoint = endPath
        ? findNearestPointOnPath(endDoor, endPath.pathPoints)
        : null;
      ...
      const pathBetween = getPathBetweenPoints(
        startPathPoint,
        endPathPoint,
        graph.walkablePaths,
        { graph }
  ...
  if (!bestOption && realStartDoors.length && realEndDoors.length) {
    ...
    if (closestPair) {
      const startPathId =
        closestPair.startDoor.nearestPathId || startRoom.nearestPathId || null;
      const endPathId =
        closestPair.endDoor.nearestPathId || endRoom.nearestPathId || null;
      const points = [
      ...
      console.warn(
        "calculateSingleFloorRoute: using synthetic straight-line fallback between rooms.",
        {
          floor: graph.floorNumber,
          startRoomId,
          endRoomId,
...
async function calculateConstrainedSameFloorRoute({
  ...
  const candidateStairKeys = determineCandidateStairKeys(
    graph,
    startPathId,
    graph,
    endPathId
  );
  const stairKeysToConsider = candidateStairKeys.length
    ? candidateStairKeys
    : (graph.stairNodes || []).map((node) => node.room.stairKey);
  const uniqueStairKeys = [...new Set(stairKeysToConsider.filter(Boolean))];
  if (!uniqueStairKeys.length) {
    console.warn("No candidate stair keys found for constrained route", {
      startPathId,
      endPathId,
    });
    return null;
  ...
  const requiredStartVariant = getRequiredStairVariantForPath(
    graph,
    startPathId
  );
  ...
  for (const stairKey of uniqueStairKeys) {
    let startStairs = findStairNodesBy(
      graph,
      (room) =>
        room.stairKey === stairKey &&
        getPrimaryPathIdForRoom(room) === startPathId
    );
    let endStairs = findStairNodesBy(
      graph,
      (room) =>
        room.stairKey === stairKey &&
        getPrimaryPathIdForRoom(room) === endPathId
    );
    ...
    for (const startStair of startStairs) {
      const startSegment = calculateSingleFloorRoute(
        graph,
        startRoomId,
        startStair.roomId
      );
      ...
      const startConnects = Array.isArray(startStair.room.connectsTo)
        ? startStair.room.connectsTo
        : [];

      for (const endStair of endStairs) {
        const sharedFloors = intersectArrays(
          startConnects,
          Array.isArray(endStair.room.connectsTo)
            ? endStair.room.connectsTo
            : []
        );
        ...
        const endSegment = calculateSingleFloorRoute(
          graph,
          endStair.roomId,
          endRoomId
        );
        ...
        for (const midFloor of sharedFloors) {
          ...
          const upperStart = findStairNodeByKeyAndVariant(
            midGraph,
            stairKey,
            startMeta.variant
          );
          const upperEnd = findStairNodeByKeyAndVariant(
            midGraph,
            stairKey,
            endMeta.variant
          );
          ...
          const middleSegment = calculateSingleFloorRoute(
            midGraph,
            upperStart.roomId,
            upperEnd.roomId
          );
            ...
            segments: [
              {
                type: "walk",
                floor: floorNumber,
                description: `Floor ${floorNumber}: Proceed to ${describeStairKey(
                  stairKey
                )}`,
              ...
              },
              {
                type: "stair",
                stairKey,
                description: `Use ${describeStairKey(
                  stairKey
                )} to reach Floor ${midFloor}`,
              ...
              },
              {
                type: "walk",
                floor: midFloor,
                description: `Floor ${midFloor}: Transition across ${describeStairKey(
                  stairKey
                )} landing`,
              ...
              },
              {
                type: "stair",
                stairKey,
                description: `Return via ${describeStairKey(
                  stairKey
  ...
  if (!bestRoute) {
    console.warn(
      "No constrained route found between rooms with enforced stair transition",
      {
        floorNumber,
        startRoomId,
        endRoomId,
...
async function calculateMultiFloorRoute(startRoomId, endRoomId, options = {}) {
  const forcedStair =
    options && typeof options === "object" ? options.forcedStair || null : null;
  const forcedParsed =
    forcedStair && forcedStair.roomId ? parseStairId(forcedStair.roomId) : null;
  const forcedStairKey = forcedStair?.stairKey || forcedParsed?.key || null;
  const forcedStairVariant =
    forcedStair?.variant || forcedParsed?.variant || null;
  const forcedAppliesTo = forcedStair?.appliesTo || "end";
  const forcedContext =
    forcedStair?.reason || options?.restrictionContext || null;
  ...
  if (startFloor == null || endFloor == null) {
    console.warn("Unable to determine floors for rooms", {
      startRoomId,
      endRoomId,
    });
    return null;
  ...
  const startRestriction = skipRestrictedStart
    ? null
    : getRestrictedAccessRule(startGraph, startRoomId);
  const endRestriction = skipRestrictedEnd
    ? null
    : getRestrictedAccessRule(endGraph, endRoomId);
  ...
  if (startRestriction) {
    ...
    if (
      startRule &&
      startRule.entryPointFloor &&
      startRule.entryPointFloor !== startFloor
    ) {
      console.log(
        `[Pre-load] Loading Floor ${startRule.entryPointFloor} for restricted start room ${startRoomId}`
      );
  ...
  if (endRestriction) {
    ...
    if (
      endRule &&
      endRule.entryPointFloor &&
      endRule.entryPointFloor !== endFloor
    ) {
      console.log(
        `[Pre-load] Loading Floor ${endRule.entryPointFloor} for restricted end room ${endRoomId}`
      );
  ...
  if (startRestriction && !skipRestrictedStart) {
    ...
    if (
      startEntry &&
      startEntry.floor === 2 &&
      startEntry.roomId === "stair_east_2-2" &&
      startFloor !== 2
    ) {
      // This is one of the restricted rooms (4-3, 5-3, or 6-3) on Floor 3
      console.log(
        `[Smart Routing] ${startRoomId} (Floor ${startFloor}) starts at ${startEntry.roomId} (Floor 2)`
      );
      ...
      console.log(
        `[Smart Routing] Destination ${endRoomId} on Floor ${endFloor}, path: ${endPathId}, isRestricted: ${destinationIsRestricted}`
      );

      // Only apply smart routing if destination is NOT restricted (to avoid conflicts with shared entry point logic)
      if (
        !destinationIsRestricted &&
        (endFloor !== startFloor || endFloor === 3)
      ) {
        // Changed condition: apply if different floor OR if same floor is Floor 3 (to route between restricted and non-restricted)

        console.log(
          `[Smart Routing] Processing route with endFloor=${endFloor}, startFloor=${startFloor}`
        );

        // Check if destination uses west or central paths - if so, use those exclusive stairs
        if (endFloor === 3 && endPathId) {
          if (
            endPathId.includes("path1") ||
            endPathId.includes("path3") ||
            endPathId.includes("lobby_vertical_1")
          ) {
            // Use west stair (stair_thirdFloor_1-2 on Floor 2)
            console.log(
              `[Smart Routing] Destination uses west path, routing via stair_thirdFloor_1-2`
            );

            // First, create Floor 2 route from stair_east_2-2 to stair_thirdFloor_1-2
            const floor2Route = calculateSingleFloorRoute(
              floor2Graph,
              "stair_east_2-2",
              "stair_thirdFloor_1-2"
            );

            // Then route from stair_thirdFloor_1-2 to destination on Floor 3
            const downstreamRoute = await calculateMultiFloorRoute(
              "stair_thirdFloor_1-2",
              endRoomId,
              {
                ...options,
            ...
            if (downstreamRoute && floor2Route) {
              ...
              downstreamRoute.segments = [
                floor2Segment,
                ...downstreamRoute.segments,
              ];
              addFloorToRoute(downstreamRoute, 2);
              downstreamRoute.totalDistance =
                (downstreamRoute.totalDistance || 0) + floor2Route.distance;
              ...
              return augmentRouteWithRestrictedStart({
                ...
                startFloor: 3,
                entryPoint: {
                  roomId: "stair_thirdFloor_1-2",
                  floor: 2,
                  room: floor2Graph.rooms["stair_thirdFloor_1-2"],
                },
              });
            }
            return null;
          } else if (
            endPathId.includes("path2") ||
            endPathId.includes("lobby_vertical_2") ||
            endPathId.includes("lobby_vertical_3")
          ) {
            // Use central stair (stair_thirdFloor_2-2 on Floor 2)
            console.log(
              `[Smart Routing] Destination uses central path, routing via stair_thirdFloor_2-2`
            );
            ...
            if (downstreamRoute && floor2Route) {
              ...
              return augmentRouteWithRestrictedStart({
                ...
                startFloor: 3,
                entryPoint: {
                  roomId: "stair_thirdFloor_2-2",
                  floor: 2,
                  room: floor2Graph.rooms["stair_thirdFloor_2-2"],
                },
              });
            }
            return null;
        ...
        if (endFloor === 2) {
          console.log(
            `[Smart Routing] Destination on Floor 2, routing via stair_east_2-2 on Floor 2`
          );
          ...
          if (downstreamRoute) {
            console.log(
              `[Smart Routing] Successfully created Floor 2 route, augmenting with restricted start`
            );
            return augmentRouteWithRestrictedStart({
              route: downstreamRoute,
              startRoomId,
            ...
            console.error(
              `[Smart Routing] Failed to create downstream route from stair_east_2-2 to ${endRoomId} on Floor 2`
            );
            return null;
          }
        }
        ...
        if (endFloor === 1) {
          console.log(
            `[Smart Routing] Destination on Floor 1, routing via stair_east_2-2 → stair_east_2-1`
          );
          ...
          if (downstreamRoute) {
            console.log(
              `[Smart Routing] Successfully created route, augmenting with restricted start`
            );
            return augmentRouteWithRestrictedStart({
              route: downstreamRoute,
              startRoomId,
            ...
            console.error(
              `[Smart Routing] Failed to create downstream route from stair_east_2-2 to ${endRoomId}`
            );
            return null;
          }
        }
        ...
        console.log(
          `[Smart Routing] No matching condition, continuing with normal flow`
        );
      } else {
        console.log(
          `[Smart Routing] Destination is restricted or same restricted floor, skipping smart routing (shared entry point logic will handle)`
        );
      }
    } else {
      console.log(
        `[Smart Routing] Start entry point is not stair_east_2-2, continuing with normal flow`
      );
    }
  } else {
    console.log(
      `[Smart Routing] No start restriction or already skipped, continuing with normal flow`
    );
  }
  ...
  if (endRestriction && !skipRestrictedEnd) {
    ...
    if (
      endEntry &&
      endEntry.floor === 2 &&
      endEntry.roomId === "stair_east_2-2" &&
      endFloor !== 2
    ) {
      console.log(
        `[Smart Routing] ${endRoomId} (Floor ${endFloor}) ends at ${endEntry.roomId} (Floor 2)`
      );
      ...
      console.log(
        `[Smart Routing] Origin ${startRoomId} on Floor ${startFloor}, path: ${startPathId}, isRestricted: ${originIsRestricted}`
      );

      // Only apply smart routing if origin is NOT restricted (to avoid conflicts with shared entry point logic)
      if (
        !originIsRestricted &&
        (startFloor !== endFloor || startFloor === 3)
      ) {
        // Changed condition: apply if different floor OR if same floor is Floor 3

        console.log(
          `[Smart Routing] Processing reverse route with startFloor=${startFloor}, endFloor=${endFloor}`
        );

        if (startFloor === 3 && startPathId) {
          if (
            startPathId.includes("path1") ||
            startPathId.includes("path3") ||
            startPathId.includes("lobby_vertical_1")
          ) {
            // Route via west stair
            console.log(
              `[Smart Routing] Origin uses west path, routing via stair_thirdFloor_1-2`
            );
            const downstreamRoute = await calculateMultiFloorRoute(
              startRoomId,
              "stair_thirdFloor_1-2",
            ...
            startPathId.includes("path2") ||
            startPathId.includes("lobby_vertical_2") ||
            startPathId.includes("lobby_vertical_3")
          ) {
            // Route via central stair
            console.log(
              `[Smart Routing] Origin uses central path, routing via stair_thirdFloor_2-2`
            );
            const downstreamRoute = await calculateMultiFloorRoute(
              startRoomId,
              "stair_thirdFloor_2-2",
        ...
        if (startFloor === 2) {
          console.log(
            `[Smart Routing] Origin on Floor 2, routing from ${startRoomId} to stair_east_2-2`
          );
          const downstreamRoute = await calculateMultiFloorRoute(
            startRoomId,
            "stair_east_2-2",
          ...
          if (downstreamRoute) {
            console.log(
              `[Smart Routing] Successfully created Floor 2 route, augmenting with restricted end`
            );
            return augmentRouteWithRestrictedEnd({
              route: downstreamRoute,
              endRoomId,
            ...
            console.error(
              `[Smart Routing] Failed to create downstream route from ${startRoomId} to stair_east_2-2 on Floor 2`
            );
            return null;
          }
        }
        ...
        if (startFloor === 1) {
          console.log(
            `[Smart Routing] Origin on Floor 1, routing via stair_east_2-1 → stair_east_2-2`
          );
          const downstreamRoute = await calculateMultiFloorRoute(
            startRoomId,
            "stair_east_2-2",
          ...
          if (downstreamRoute) {
            console.log(
              `[Smart Routing] Successfully created route, augmenting with restricted end`
            );
            return augmentRouteWithRestrictedEnd({
              route: downstreamRoute,
              endRoomId,
            ...
            console.error(
              `[Smart Routing] Failed to create downstream route from ${startRoomId} to stair_east_2-2`
            );
            return null;
          }
        }
        ...
        console.log(
          `[Smart Routing] No matching condition for reverse route, continuing with normal flow`
        );
      } else {
        console.log(
          `[Smart Routing] Origin is restricted or same restricted floor, skipping smart routing (shared entry point logic will handle)`
        );
      }
    } else {
      console.log(
        `[Smart Routing] End entry point is not stair_east_2-2, continuing with normal flow`
      );
    }
  } else {
    console.log(
      `[Smart Routing] No end restriction or already skipped, continuing with normal flow`
    );
  }
  ...
  if (
    ...
    if (
      ...
      console.log(
        `Both ${startRoomId} and ${endRoomId} use same entry point ${startEntry.roomId} on Floor ${startEntry.floor} - routing on entry floor only`
      );
      ...
      if (entryFloorGraph && entryFloorGraph.rooms[startEntry.roomId]) {
        ...
        // Ensure entryRoom has roomId for door filtering
        if (!entryRoom.roomId) {
          entryRoom.roomId = startEntry.roomId;
        }
  ...
  if (startRestriction && !skipRestrictedStart) {
    ...
    if (
      entryPoint &&
      entryPoint.floor === 2 &&
      entryPoint.roomId === "stair_east_2-2"
    ) {
      ...
      console.log(
        `Multi-floor: Start room ${startRoomId} restricted, using entry point ${entryPoint.roomId} on floor ${entryPoint.floor}`
      );
  ...
  if (endRestriction && !skipRestrictedEnd) {
    ...
    if (
      ...
      console.log(
        `Multi-floor: End room ${endRoomId} restricted, using entry point ${entryPoint.roomId} on floor ${entryPoint.floor}`
      );
      ...
      const routeToEntry = await calculateMultiFloorRoute(
        startRoomId,
        entryPoint.roomId,
        forcedOptions
      );
  ...
  if (startFloor === endFloor) {
    ...
    const startEntry = startRestriction
      ? getMandatoryEntryPoint(graph, startRoomId)
      : null;
    const endEntry = endRestriction
      ? getMandatoryEntryPoint(graph, endRoomId)
      : null;
    const startRequiresCrossFloor =
      startEntry && startEntry.floor !== startFloor;
    ...
    const hasCrossFloorRestriction =
      (startRestriction &&
        getMandatoryEntryPoint(graph, startRoomId)?.floor !== startFloor) ||
      (endRestriction &&
        getMandatoryEntryPoint(graph, endRoomId)?.floor !== endFloor);
  ...
  for (let i = 0; i < floorRange.length - 1; i++) {
    ...
    if (!transitions.length) {
      console.warn(
        "No stair transitions available between floors",
        floorA,
        floorB
      );
      return null;
  ...
  let constrainedStairKeys = determineCandidateStairKeys(
    startGraph,
    startPathId,
    endGraph,
    endPathId
  ...
  const rawRequiredEndVariant = getRequiredStairVariantForPath(
    endGraph,
    endPathId
  );
  ...
  if (forcedKeyActive && forcedStairKey) {
    if (
      ...
      !constrainedStairKeys.includes(forcedStairKey)
    ) {
      ...
      console.log(
        "Forced stair provided without existing constraints; keeping transitions unconstrained.",
        {
          forcedStairKey,
  ...
  if (constrainedStairKeys && constrainedStairKeys.length) {
    for (let i = 0; i < transitionsPerStep.length; i++) {
      ...
      const enforceStartVariant = !(
        forcedKeyActive &&
        forcedAppliesTo !== "start" &&
        requiredStartVariant &&
        requiredStartVariant.stairKey !== forcedStairKey
      );
      ...
      transitionsPerStep[i] = transitionsPerStep[i].filter((transition) => {
        ...
        if (!constrainedStairKeys.includes(transition.stairKey)) {
          console.log(
            `Rejecting transition ${transition.startNode.roomId} -> ${transition.endNode.roomId}: stairKey ${transition.stairKey} not in allowed keys`,
            constrainedStairKeys
          );
          return false;
        }
        ...
        if (
          ...
          enforceStartVariant &&
          requiredStartVariant &&
          transition.stairKey === requiredStartVariant.stairKey
        ) {
          const startParsed = parseStairId(transition.startNode.roomId);
          if (
            !startParsed ||
            startParsed.variant !== requiredStartVariant.variant
          ) {
            console.log(
              `Rejecting first transition ${transition.startNode.roomId}: variant ${startParsed?.variant} doesn't match required ${requiredStartVariant.variant}`
            );
            return false;
          }
        }
        ...
        if (
          isLastTransition &&
          effectiveEndVariant &&
          transition.stairKey === effectiveEndVariant.stairKey
        ) {
          const endParsed = parseStairId(transition.endNode.roomId);
          if (!endParsed || endParsed.variant !== effectiveEndVariant.variant) {
            console.log(
              `Rejecting last transition ${transition.endNode.roomId}: variant ${endParsed?.variant} doesn't match required ${effectiveEndVariant.variant}`
            );
            return false;
          }
        }
        ...
        if (
          ...
          if (
            ...
            console.log(
              `Rejecting transition start ${transition.startNode.roomId}: variant ${startParsed.variant} doesn't match required ${requiredStartVariant.variant}`
            );
            return false;
          }
          if (endParsed && endParsed.variant !== requiredStartVariant.variant) {
            console.log(
              `Rejecting transition end ${transition.endNode.roomId}: variant ${endParsed.variant} doesn't match required ${requiredStartVariant.variant}`
            );
            return false;
          }
        }
        ...
        if (
          effectiveEndVariant &&
          effectiveEndVariant.stairKey !== requiredStartVariant?.stairKey &&
          transition.stairKey === effectiveEndVariant.stairKey
        ) {
          const startParsed = parseStairId(transition.startNode.roomId);
          const endParsed = parseStairId(transition.endNode.roomId);
          ...
          if (
            startParsed &&
            startParsed.variant !== effectiveEndVariant.variant
          ) {
            console.log(
              `Rejecting transition start ${transition.startNode.roomId}: variant ${startParsed.variant} doesn't match end required ${effectiveEndVariant.variant}`
            );
            return false;
          }
          if (endParsed && endParsed.variant !== effectiveEndVariant.variant) {
            console.log(
              `Rejecting transition end ${transition.endNode.roomId}: variant ${endParsed.variant} doesn't match end required ${effectiveEndVariant.variant}`
            );
            return false;
          }
        }
        ...
        console.log(
          `Accepting transition ${transition.startNode.roomId} -> ${transition.endNode.roomId}`
        );
        return true;
      });
  ...
  const evaluateTransitions = (
    ...
    if (stepIndex >= transitionsPerStep.length) {
      ...
      const finalRoute = calculateSingleFloorRoute(
        finalGraph,
        currentRoomId,
        endRoomId
      );
      ...
      if (finalRoute.distance > 0) {
        finalSegments.push(
          createWalkSegment(
            currentFloor,
            `Floor ${currentFloor}: Continue to ${endRoomId}`,
            finalRoute
          )
        );
    ...
    transitions.forEach((transition) => {
      ...
      const approachRoute = calculateSingleFloorRoute(
        graph,
        currentRoomId,
        transition.startNode.roomId
      );
      if (!approachRoute) {
        return;
      }
      ...
      if (approachRoute.distance > 0) {
        const stairLabel =
          transition.startNode.label || transition.startNode.roomId;
        updatedSegments.push(
          createWalkSegment(
            currentFloor,
            `Floor ${currentFloor}: Proceed to ${stairLabel}`,
            approachRoute,
            transition.stairKey
          )
        );
  ...
  const evaluatedRoute = evaluateTransitions(
    0,
    startFloor,
    startRoomId,
    [],
  ...
  if (!evaluatedRoute) {
    console.warn("Unable to evaluate a viable multi-floor route", {
      startRoomId,
      endRoomId,
      floorRange,
    });
...
async function activateRouteBetweenRooms(startRoomId, endRoomId) {
  // CRITICAL FIX: Clean all SVG path groups BEFORE calculating route
  // This prevents transform corruption from previous paths interfering with new calculations
  try {
    const svg = getCapitolSVG();
    if (svg) {
      const mainGroup = svg.querySelector(".svg-pan-zoom_viewport") || svg.querySelector("g");
      if (mainGroup) {
        // Clean path highlight group
        const pathGroup = mainGroup.querySelector("#path-highlight-group");
        if (pathGroup) {
          console.log("🔧 Pre-pathfinding cleanup: Removing transform from path group");
          pathGroup.removeAttribute("transform");
          
          // Scan for any paths with corrupt transforms
          const paths = pathGroup.querySelectorAll("path");
          paths.forEach(path => {
            if (path.hasAttribute("transform")) {
              console.log("🔧 Pre-pathfinding cleanup: Removing transform from path element");
              path.removeAttribute("transform");
            }
          });
        }
        
        // Clean any marker groups that might have corrupt transforms
        const markerGroups = mainGroup.querySelectorAll('g[id*="marker"], g[id*="label"]');
        markerGroups.forEach(group => {
          const transform = group.getAttribute("transform");
          if (transform && (transform.includes("NaN") || transform.includes("undefined") || !transform.includes(")"))) {
            console.log("🔧 Pre-pathfinding cleanup: Removing corrupt transform from marker group:", group.id);
            group.removeAttribute("transform");
          }
        });
        
        console.log("✅ Pre-pathfinding SVG cleanup completed");
      }
    }
  } catch (cleanupError) {
    console.warn("⚠️ Pre-pathfinding cleanup encountered an error (non-fatal):", cleanupError);
  }
...
async function roomClickHandler(event) {
  ...
  console.log(
    "Available room IDs:",
    graph && graph.rooms ? Object.keys(graph.rooms) : "No rooms"
  );
  ...
  if (!graph || !graph.rooms) {
    ...
    alert(
      "Navigation data not loaded yet. Please wait a moment and try again."
    );
    return;
  }
  ...
  if (!clickedRoom) {
    ...
    alert(
      `Room "${roomId}" not found in navigation data. Please try a different room.`
    );
    return;
  }
  ...
  if (entryPoints.length === 0 && !isRestrictedRoom) {
    console.error(
      "No entry points found for this room and no restriction rules"
    );
    return;
  }
  ...
  const entryPointsToUse =
    realEntryPoints.length > 0 ? realEntryPoints : entryPoints;
  ...
  if (entryPointsToUse.length > 0) {
    ...
    if (clickedRoom.nearestPathId) {
      const clickedPath = graph.walkablePaths.find(
        (p) => p.id === clickedRoom.nearestPathId
      );
      if (clickedPath && nearestEntryPoint) {
        clickedPathPoint = findNearestPointOnPath(
          nearestEntryPoint,
          clickedPath.pathPoints
        );
      }
    }
  }
  ...
  if (selectedRooms.length === 1) {
    ...
    if (clickedPathPoint && nearestEntryPoint && !nearestEntryPoint.virtual) {
      drawCompletePath([nearestEntryPoint, clickedPathPoint]);
    } else {
      console.log(
        "First room selected (restricted or no path on this floor):",
        roomId
      );
    ...
    console.log(
      "Attempting to compute route between rooms:",
      startRoomId,
      "->",
      endRoomId
    );
    ...
    try {
      ...
      if (!route) {
        console.error(
          "No route could be calculated between the selected rooms."
        );
        alert(
          "No available route between the selected rooms. Please verify that stair data exists for both floors."
        );
        return;
      }
      ...
      if (route.type === "multi-floor") {
        const stairNames =
          Array.isArray(route.stairSequence) && route.stairSequence.length
            ? [
                ...new Set(
                  route.stairSequence.map((item) =>
                    describeStairKey(item.stairKey)
                  )
                ),
              ]
            : route.stairKey
        ...
        if (stairNames.length) {
          console.log(
            `Multi-floor route computed using ${stairNames.join(", ")}.`
          );
        } else {
      ...
      alert(
        "Unable to calculate a route between the selected rooms. Please try again."
      );
    }
  }
}
...
function aStar(start, goal) {
  ...
  while (openSet.length > 0 && iterations < maxIterations) {
    ...
    console.log(
      `Iteration ${iterations}: Current node = ${current}, Open set size = ${openSet.length}`
    );
    ...
    if (current === goal) {
      ...
      if (pathLength >= maxPathLength) {
        console.error(
          "Path reconstruction exceeded maximum length - possible circular reference"
        );
        return null;
      }
  ...
  path.forEach((roomId, index) => {
    ...
    if (el) {
      ...
      if (el.tagName === "G") {
        ...
        if (pathElement) {
          console.log(
            "Found path element inside group, highlighting path instead"
          );
...
function clearAllPaths() {
  ...
  const pathGroup = mainGroup.querySelector("#path-highlight-group");
  if (pathGroup) {
    // CRITICAL FIX: Remove transform attribute BEFORE clearing children
    if (pathGroup.hasAttribute("transform")) {
      console.log("🔧 clearAllPaths: Removing corrupt transform from path group");
      pathGroup.removeAttribute("transform");
    }
  ...
  // Clear "YOU ARE HERE" highlights and labels
  document.querySelectorAll('.you-are-here').forEach(el => {
    el.classList.remove('you-are-here');
  });
  document.querySelectorAll('.you-are-here-label').forEach(label => {
    label.remove();
  });
  console.log("Cleared YOU ARE HERE markers from clearAllPaths");
}

function getPathBetweenPoints(start, end, walkablePaths, options = {}) {
  const graphContext =
    options.graph || (window.MOBILE_MODE ? window.floorGraph : floorGraph);
  ...
  walkablePaths.forEach((path) => {
    ...
    path.pathPoints.forEach((point) => {
      ...
      if (previousPoint) {
        const prevPointId = `${Math.round(previousPoint.x * 100) / 100},${
          Math.round(previousPoint.y * 100) / 100
        }`;
  ...
  const startId = `${Math.round(start.x * 100) / 100},${
    Math.round(start.y * 100) / 100
  }`;
  const endId = `${Math.round(end.x * 100) / 100},${
...
function aStarSearch(graph, startId, endId) {
  ...
  while (openSet.size > 0 && iterations < maxIterations) {
    ...
    for (const [neighborId, distance] of Object.entries(neighbors)) {
      ...
      fScore[neighborId] =
        tentativeG + getDistance(graph[neighborId], graph[endId]);
      console.log(
        `Updated node ${neighborId} with g=${tentativeG}, f=${fScore[neighborId]}`
      );
    }
  }
...
function drawCompletePath(points, options = {}) {
  ...
  console.log("🎨 drawCompletePath CALLED with", points?.length, "points");
  console.log("🎨 Options:", options);
  console.log("🎨 Call stack:", new Error().stack);
  
  // Wrap entire function in try-catch to handle any SVG corruption errors
  try {
  ...
  if (validPoints.length !== points.length) {
    console.warn(
      "Some points have invalid coordinates, using only valid points:",
      validPoints
    );
  }
  ...
  const filteredPoints = validPoints.filter((point, index, arr) => {
    ...
    // Calculate slopes
    const slope1 =
      Math.abs(point.y - prev.y) < 0.1
        ? Infinity
        : (point.x - prev.x) / (point.y - prev.y);
    const slope2 =
      Math.abs(next.y - point.y) < 0.1
        ? Infinity
  ...
  if (!pathGroup) {
    ...
    // Ensure path group has no transform attribute
    pathGroup.removeAttribute("transform");
    mainGroup.appendChild(pathGroup);
    console.log("Created new path highlight group");
  } else {
    console.log("Using existing path highlight group");
    // Safety: Remove any corrupted transform from existing group
    pathGroup.removeAttribute("transform");
  }
  ...
  if (clearExisting) {
    ...
    // Clear "YOU ARE HERE" highlights and labels when starting a new path
    document.querySelectorAll('.you-are-here').forEach(el => {
      el.classList.remove('you-are-here');
    });
    document.querySelectorAll('.you-are-here-label').forEach(label => {
    ...
    console.log("Cleared YOU ARE HERE markers");
  }
  
  // Final safety check: Ensure path group transform is clean before adding paths
  if (pathGroup.hasAttribute("transform")) {
    console.warn("⚠️ Path group had corrupt transform, removing it");
    pathGroup.removeAttribute("transform");
  }
  ...
  // Generate the path data with validation
  let pathData = "";
  filteredPoints.forEach((point, index) => {
    // Validate coordinates to prevent corruption
  ...
  // Validate path data before setting
  if (!pathData || pathData.length < 5) {
    console.error("Invalid path data generated, aborting path drawing");
    return;
  }
  ...
  // Ensure no transform attribute on the path element itself
  ...
  const dashCycleDistance =
    dashSegments.reduce((sum, value) => sum + value, 0) || 15;
  ...
  // Add START and DESTINATION labels at the door points
  if (filteredPoints.length >= 2) {
    const startPoint = filteredPoints[0];
    const endPoint = filteredPoints[filteredPoints.length - 1];
    
    // Validate coordinates before creating labels
    if (!isNaN(startPoint.x) && !isNaN(startPoint.y) && 
        !isNaN(endPoint.x) && !isNaN(endPoint.y)) {
      
      // Create START label with bounce animation
      const startLabel = document.createElementNS("http://www.w3.org/2000/svg", "text");
      startLabel.setAttribute("x", startPoint.x);
      startLabel.setAttribute("y", startPoint.y - 20); // 20px above the point
      startLabel.setAttribute("text-anchor", "middle");
      startLabel.setAttribute("font-family", "'Segoe UI', Arial, sans-serif");
      startLabel.setAttribute("font-size", "16");
      startLabel.setAttribute("font-weight", "bold");
      startLabel.setAttribute("fill", "#1a5632");
      startLabel.setAttribute("stroke", "#ffffff");
      startLabel.setAttribute("stroke-width", "4");
      startLabel.setAttribute("paint-order", "stroke fill");
      startLabel.setAttribute("class", "route-label start-label");
      startLabel.removeAttribute("transform"); // Ensure no transform corruption
      startLabel.style.animation = "bounce 2s infinite";
      startLabel.style.vectorEffect = "non-scaling-stroke";
      startLabel.textContent = "START";
      pathGroup.appendChild(startLabel);
      
      // Create DESTINATION label with bounce animation
      const endLabel = document.createElementNS("http://www.w3.org/2000/svg", "text");
      endLabel.setAttribute("x", endPoint.x);
      endLabel.setAttribute("y", endPoint.y - 20); // 20px above the point
      endLabel.setAttribute("text-anchor", "middle");
      endLabel.setAttribute("font-family", "'Segoe UI', Arial, sans-serif");
      endLabel.setAttribute("font-size", "16");
      endLabel.setAttribute("font-weight", "bold");
      endLabel.setAttribute("fill", "#dc3545");
      endLabel.setAttribute("stroke", "#ffffff");
      endLabel.setAttribute("stroke-width", "4");
      endLabel.setAttribute("paint-order", "stroke fill");
      endLabel.setAttribute("class", "route-label destination-label");
      endLabel.removeAttribute("transform"); // Ensure no transform corruption
      endLabel.style.animation = "bounce 2s infinite";
      endLabel.style.vectorEffect = "non-scaling-stroke";
      endLabel.textContent = "DESTINATION";
      pathGroup.appendChild(endLabel);
      
      console.log("Added START and DESTINATION labels with bounce animation to path");
    } else {
      console.warn("Invalid coordinates for START/DESTINATION labels", {startPoint, endPoint});
    }
  }
  
  // CRITICAL DEBUG: Confirm successful completion
  console.log("✅ ========== drawCompletePath COMPLETED SUCCESSFULLY ==========");
  console.log("✅ Path element:", pathElement);
  console.log("✅ Path is in DOM:", document.contains(pathElement));
  console.log("✅ Path parent:", pathElement.parentNode);
  console.log("✅ Path visibility:", pathElement.style.visibility || 'default');
  console.log("✅ Path display:", pathElement.style.display || 'default');
  console.log("✅ Path opacity:", pathElement.style.opacity || 'default');
  console.log("✅ Path stroke:", pathElement.getAttribute('stroke'));
  console.log("✅ Path stroke-width:", pathElement.getAttribute('stroke-width'));
  console.log("✅ =============================================================");
  
  } catch (error) {
    console.error("❌ Error in drawCompletePath - SVG corruption detected:", error);
    ...
    // Try to recover by clearing the path group
    try {
      const svg = getCapitolSVG();
      if (svg) {
        const mainGroup = svg.querySelector(".svg-pan-zoom_viewport") || svg.querySelector("g");
        if (mainGroup) {
          const pathGroup = mainGroup.querySelector("#path-highlight-group");
          if (pathGroup) {
            console.log("🔧 Attempting to clear corrupted path group...");
            pathGroup.innerHTML = "";
            pathGroup.removeAttribute("transform");
            console.log("✅ Path group cleared and reset");
          }
        }
      }
    } catch (recoveryError) {
      console.error("Failed to recover from path drawing error:", recoveryError);
    }
    
    // Alert user about the issue
    if (typeof window !== 'undefined' && window.alert) {
      alert("Unable to draw path. Please try refreshing the page and selecting the rooms again.");
    }
  }
}
...
function drawPathLines(path) {
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (graph.rooms[currentRoomId] && graph.rooms[nextRoomId]) {
      ...
      console.warn(
        `Could not find room data for ${currentRoomId} or ${nextRoomId}`
      );
    }
  }
